...
Detect and react to an identity-aware reverse proxy (e.g. Cloudflare Access) silently expiring a user's session, so the web client shows a clear re-login prompt instead of appearing broken (failed WebSocket reconnects, stale cached assets, or hangs).
